/* french-manga - built 2026-07-09T18:46:56.598Z */
...
function resolveBase() {
  return __async(this, null, function* () {
    ...
    var r = yield safeFetch(ROOT_REDIRECT, { method: "GET" }, 8e3);
    if (r && r.url) {
      var m = /^(https?:\/\/[^/]+)/i.exec(r.url);
      if (m && /french-manga\.net/i.test(m[1])) {
        cachedBase = m[1];
        return cachedBase;
      }
    }
    ...
    for (var i = 0; i < FALLBACK_HOSTS.length; i++) {
      var rr = yield safeFetch(FALLBACK_HOSTS[i] + "/", { method: "GET" }, 6e3);
      if (rr && rr.ok) {
