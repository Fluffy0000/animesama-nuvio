 ...
 * Generated: 2026-07-09T09:33:59.321Z
 */
...
function resolveVidmoly(embedUrl) {
  return __async(this, null, function* () {
    const html = yield fetchText(embedUrl, { headers: { "Referer": BASE_URL + "/", "User-Agent": USER_AGENT } }, 8e3);
    const file = firstMatch(html, /sources:\s*\[\s*\{\s*file:\s*["']([^"']+)["']/i) || firstMatch(html, /file:\s*["'](https?:\/\/[^"']+\.m3u8[^"']*)["']/i);
    if (!file) return null;
    return {
      url: file,
      quality: /master\.m3u8/i.test(file) ? "Auto" : "HD",
  ...
  return __async(this, null, function* () {
    ...
    for (let i = 0; i < candidates.length; i += BATCH) {
      ...
      const parts = yield Promise.all(batch.map((c) => __async(null, null, function* () {
        ...
        return {
          name: "Anime-Sama",
          title: c.player + " - Ep " + c.episode + " - " + c.lang,
          url: direct.url,
...
function findCandidates(tmdbId, mediaType, season, episode, titles, deadline) {
  return __async(this, null, function* () {
    ...
    let cands = yield raceSlugs(slugs, s, ep, isMovie, deadline);
