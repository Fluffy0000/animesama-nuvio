 ...
 * Generated: 2026-07-09T10:55:05.597Z
 */
...
var RESOLVABLE_PLAYERS = { Vidmoly: 1, Sibnet: 1, Smoothpre: 1, Sendvid: 1 };
var PREFERRED_PLAYERS = ["Vidmoly", "Smoothpre", "Sibnet", "Sendvid"];
...
function buildCandidates(parsedArrays, lang, index, label) {
  ...
  for (const arr of parsedArrays) {
    ...
    if (!RESOLVABLE_PLAYERS[player]) continue;
    seen[url] = true;
    streams.push({
      embedUrl: url,
      player,
...
function fetchHlsInfo(masterUrl, referer) {
  return __async(this, null, function* () {
    try {
      const pl = yield fetchText(masterUrl, { headers: { "Referer": referer, "User-Agent": USER_AGENT } }, 6e3);
...
function resolveVidmoly(embedUrl) {
  return __async(this, null, function* () {
    ...
    if (!file) return null;
    const info = yield fetchHlsInfo(file, "https://vidmoly.biz/");
...
function resolveSmoothpre(embedUrl) {
  return __async(this, null, function* () {
    const html = yield fetchText(embedUrl, { headers: { "Referer": getBase() + "/", "User-Agent": USER_AGENT } }, 8e3);
    let file = firstMatch(html, /sources:\s*\[\s*\{\s*file:\s*["']([^"']+)["']/i) || firstMatch(html, /file:\s*["'](https?:\/\/[^"']+\.m3u8[^"']*)["']/i);
    if (!file) {
      const un = unpackPacked(html);
      file = firstMatch(un, /file:\s*["']([^"']+\.m3u8[^"']*)["']/i) || firstMatch(un, /sources:\s*\[\s*\{\s*file:\s*["']([^"']+)["']/i) || firstMatch(un, /(https?:\/\/[^"'\\ ]+\.m3u8[^"'\\ ]*)/i);
    }
    if (!file) return null;
    const info = yield fetchHlsInfo(file, "https://smoothpre.com/");
    if (!info.ok) return null;
    return {
      url: file,
      quality: qualityLabel(info.height),
...
function resolveEmbed(embedUrl, player) {
  return __async(this, null, function* () {
    try {
      ...
      if (player === "Smoothpre") return yield resolveSmoothpre(embedUrl);
    } catch (e) {
    }
    return null;
  });
...
function resolveStreams(candidates) {
  return __async(this, null, function* () {
    ...
    for (let i = 0; i < candidates.length; i += BATCH) {
      ...
      const parts = yield Promise.all(batch.map((c) => __async(null, null, function* () {
        const direct = yield resolveEmbed(c.embedUrl, c.player);
        if (!direct || !direct.url) return null;
        const isHls = direct.url.toLowerCase().indexOf(".m3u8") !== -1;
        if (!isHls && !(yield isLinkAlive(direct.url, direct.headers))) {
    ...
    resolved.sort((a, b) => langRank(a.language) - langRank(b.language) || b.height - a.height || playerRank(a.player) - playerRank(b.player));
