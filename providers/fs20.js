...
function withDefaultHeaders(h) {
  ...
  // NOTE: do NOT set Accept-Encoding (OkHttp decodes gzip only when absent). Match Anime-Sama.
...
function fetchOnce(url, opts, timeoutMs) {
  ...
  // REAL timeout via Promise.race (Nuvio ignores AbortController, so this is the only thing that
  // actually bounds a hanging request). If the fetch hangs, the timer wins -> returns null ->
  // the provider moves on instead of freezing the whole run. Matches the working reference impl.
