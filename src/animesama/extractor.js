...
export function extractPanels(html, slug) {
  const panels = [];
  const seen = {};
  // Match the double-quoted args directly. [^"]* keeps apostrophes inside the
  // name (e.g. "Train de l'infini") and — unlike a regex backreference — works
  // on Hermes (mobile), whose regex engine rejects backreferences.
...
function extractFilmNames(html) {
  const names = [];
  // Double-quoted match: keeps apostrophes (l'Infini) and is Hermes-safe (no
  // backreference).
