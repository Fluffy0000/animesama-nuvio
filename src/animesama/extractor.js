...
async function resolveVidmoly(embedUrl) {
  const html = await fetchText(embedUrl, { headers: { "Referer": BASE_URL + "/", "User-Agent": USER_AGENT } }, 8000);
  const file = firstMatch(html, /sources:\s*\[\s*\{\s*file:\s*["']([^"']+)["']/i) ||
    firstMatch(html, /file:\s*["'](https?:\/\/[^"']+\.m3u8[^"']*)["']/i);
  if (!file) return null;
  return { url: file, quality: /master\.m3u8/i.test(file) ? "Auto" : "HD",
...
async function resolveSendvid(embedUrl) {
  ...
  if (!file) return null;
  return { url: file.replace(/&amp;/g, "&"), quality: "HD",
...
async function findCandidates(tmdbId, mediaType, season, episode, titles, deadline) {
  ...
  let cands = await raceSlugs(slugs, s, ep, isMovie, deadline);
